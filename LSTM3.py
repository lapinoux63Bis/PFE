...
df_train = pd.read_csv(r"C:\Users\Administrator\Documents\EN_ENSEIGNE\PFE\RPI\DATASETS\Normal_1\cpu_stats.csv")
df_test = pd.read_csv(r"C:\Users\Administrator\Documents\EN_ENSEIGNE\PFE\RPI\DATASETS\SSH_bruteforce1\cpu_stats.csv")
...
for i in range(n_features):
    ...
    plt.plot(y_test[:, i], label=f'{df_train.columns[i+1]} réelle')
    plt.plot(y_pred[:, i], label=f'{df_train.columns[i+1]} prédite')
    plt.legend()
    plt.title(f'Prédiction de {df_train.columns[i+1]}')
    plt.show()